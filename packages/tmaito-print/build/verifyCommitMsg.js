...
const commitRE = /^(v\d+\.\d+\.\d+(-(alpha|beta|rc.\d+))?)|(Merge (\(.+\))?)|((revert: )?(feat|fix|docs|style|config|refactor|perf|test|ci|chore|build)(\(.+\))?!?: .{1,50})/;
if (!commitRE.test(msg)) {
  console.error(
    `  ${chalk.bgRed.white(" ERROR ")} ${chalk.red(
      ...
      `    ${chalk.green(`config(:wrench:): 修改配置`)}\n` +
      `    ${chalk.green(
      ...
      `    ${chalk.green(
        `build(:rocket:): 改变构建流程，新增依赖库和工具`
      )}\n\n` +
      chalk.red(
        `See https://zmh7057.github.io/learning-JavaScript/#/gitmoji for more details.\n`
      )
  );
  process.exit(1);
}
